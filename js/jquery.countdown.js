...
jQuery.fn.countdown = function(userOptions)
{
  ...
  var digits = [], intervals = [],$this = this;
  ...
  var makeMovement = function(elem, steps, isForward)
  {
    ...
    intervals[elem] = setInterval(function(){
      ...
      //获取当前时间值
      var times = $.map(digits, function(n){
          return n.current;
      });
      times.splice(2,0,":");
      //执行每秒步进回调
      options.step(times.join(""));
  };
  ...
  var moveDigit = function(elem)
  {
    ...
    //启动
    jQuery.fn.countdown.start = function(){
        intervals.main = setInterval(function(){ moveDigit(digits.length - 1); },1000);
    };
    //暂停
    jQuery.fn.countdown.pause = function(){
        clearInterval(intervals.main);
    };
    //更新
    jQuery.fn.countdown.set = function(time){
        var c = 0;
        digits = [];
        $this.empty();
        for (var i = 0; i < time.length; i++){
            ...
            {
                elem = $('<div class="cntSeparator"/>').css({float: 'left'}).text(time[i]);
            }
            $this.append(elem)
        }
    };
